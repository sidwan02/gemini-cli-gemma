...
export const GemmaAgent = (
  config: Config,
): LocalAgentDefinition<typeof GemmaAgentOutputSchema> => ({
  name: 'gemma_agent',
  kind: 'local',
  ...
  modelConfig: {
    model: config.getGemmaSubagentSettings().model || 'gemma3n:e2b',
    host: config.getGemmaSubagentSettings().host || 'http://localhost:11434',
    temp: 0.1,
    top_p: 0.95,
  ...
  runConfig: {
    maxTimeMinutes: config.getGemmaSubagentSettings().maxTimeMinutes || 5,
    maxTurns: config.getGemmaSubagentSettings().maxNumTurns || 15,
  },
